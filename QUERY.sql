...
CREATE TABLE Matches (
    ...
    constraint chk_matches_negativeTicketPrice check(base_ticket_price >= 0),
...
CREATE TABLE Bookings (
    ...
    constraint fk_bookings_match_id foreign key(match_id) references matches(match_id),
    -- Write your check constraint to ensure 'total_cost' is non-negative
    constraint chk_bookings_totalCostNotNegative check(total_cost>=0),
    -- Write your check constraint to restrict 'payment_status' values
    constraint chk_bookings_paymentStatus check(payment_status in ('Pending','Confirmed','Cancelled','Refunded'))
);
...
select booking_id, match_id, total_cost from matches inner join bookings using(match_id)
